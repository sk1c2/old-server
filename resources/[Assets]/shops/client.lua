...
local twentyfourseven_shops = {
	{ ['x'] = 1961.1140136719, ['y'] = 3741.4494628906, ['z'] = 32.34375 , ['r'] = 290.0},
...
function setShopBlip()
	...
	for k,v in ipairs(twentyfourseven_shops)do
		-- modelHasha = GetHashKey("a_m_m_prolhost_01")
		-- RequestModel(modelHasha)
		-- while not HasModelLoaded(modelHasha) do
		-- 	 Wait(1)
		-- end
		-- created_ped = CreatePed(0, modelHasha , v.x - 2  , v.y , v.z - 1,true)
	...
	for k,v in ipairs(tool_shops)do
		...
		modelHash = GetHashKey("s_m_y_armymech_01")
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if(Vdist(-1193.26, -892.30, 99.99, pos.x, pos.y, pos.z) < 10.0)then
			found = true
			DrawMarker(27, -1193.26, -892.30, 99.99 - 1, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 0, 25, 165, 165, 0,0, 0,0)
		...
		if(Vdist(2806.45, 5978.82, 350.70, pos.x, pos.y, pos.z) < 10.0)then
			found = true
			DrawMarker(27, 2806.45, 5978.82, 350.70 - 1, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 0, 25, 165, 165, 0,0, 0,0)
			if(Vdist(2806.45, 5978.82, 350.70, pos.x, pos.y, pos.z) < 5.0)then
				DisplayHelpText("Press ~INPUT_CONTEXT~ what dis?")
				if IsControlJustPressed(1, 38) then

...
function Menu.addButton(name, func,args,extra,damages,bodydamages,plate)
	...
	if plate ~= nil then
		Menu.GUI[Menu.buttonCount+1]["plate"] = plate
	end
	if damages ~= nil then
...
function Menu.renderButtons()
	...
	for id, settings in pairs(Menu.GUI) do
		...
		DrawRect(0.598, settings["ymin"], 0.06, settings["ymax"]-0.002, 255,255,102,0)
		...
		if(settings["plate"] ~= nil and settings["active"]) then
			SetTextFont(4)

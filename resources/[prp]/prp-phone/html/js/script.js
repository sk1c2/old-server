...
$(document).ready(function() {
    ...
    $.post('http://wrp-phone/getWeather', JSON.stringify({}));
    ...
    document.onkeyup = function(data) {
        ...
        if (data.which == 27) {
            $.post('http://wrp-phone/close', JSON.stringify({}));
        }
    }
        ...
        if (action !== undefined) {
            switch (action) {
                case "yellow-pages-delete":
                    $.post('http://wrp-phone/deleteYP', JSON.stringify({}));
                    break;
                case "racing-create":
                ...
                case "newPostSubmit":
                    e.preventDefault();
                    $.post('http://wrp-phone/newPostSubmit', JSON.stringify({
                    ...
                    break;
                case "group-manage":
                    $.post('http://wrp-phone/manageGroup', JSON.stringify({ GroupID: $(this).data('action-data'), GroupID2: $(this).data('action-data-name'), GroupID3: $(this).data('action-data-rank') }));
                    break;
                case "btnTaskGang":
                    manageGroup = $(this).data('action-data');
                    $.post('http://wrp-phone/btnTaskGang', JSON.stringify({}));
                    break;
                ...
                case "getCallHistory":
                    if (callStates[currentCallState] === "isCallInProgress" && currentContainer !== "incoming-call")
                        ...
                        $.post('http://wrp-phone/' + action, JSON.stringify({}));
                    break;
                case "dabcoin":
            ...
            case "racing:hud:update":
                switch (item.hudState) {
                    ...
                    case "finished":
                        ...
                        drawRaceStats();
                        $.post('http://wrp-phone/race:completed', JSON.stringify({
...
function openBrowser(url) {
    ...
    closePhoneShell();
    $.post('http://wrp-phone/btnCamera', JSON.stringify({}));
    $("#browser").fadeIn(300);
}
                    ...
                    <div class="row">
                        <div class="col s12"> 
                            <ul class="collection">
                                ...
                                <li class="collection-item"><i class="fas fa-hourglass-half"></i> ${vehicleData[vehicle].payments == 0 ? 'No remaining payments.' : Math.ceil(parseFloat(ConvertMinutes(vehicleData[vehicle].lastPayment))) + ' days until payment is due.'}</li>
                                `
...
function addAccountInformation(accountInfo) {
    if (accountInfo) {
        ...
        var license2 = accountInfo.license2
        // // let licensesObject = {}
        ...
        for (const key of Object(licenses)) {
            var string = `Weapons License`
            var string2 = `Driving License`
        ...
        `
        $('.account-information-secondary-job').text("No secondary job.");
...
function addMessage(item) {
    ...
    element.click(function () {
        $.post('http://wrp-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: item.msgDisplayName }));
    });
...
function addMessageOther(item) {
    ...
    element.id = item.id;
    element.click(function () {
        $.post('http://wrp-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: receiver, clientPhone: item.clientNumber }));
    });
    $(".messages-entries").prepend(element);
}
...
function updateSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            ...
            break;
        case "control":
            $.post('http://wrp-phone/settingsUpdateToko', JSON.stringify({tag: "controlUpdate",controls: currentBinds}));
            break;
        case "browser":
            break;
        default:
...
function ResetSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            $.post('http://wrp-phone/settingsResetToko', JSON.stringify());
            break;
        case "control":
            $.post('http://wrp-phone/settingsResetControls', JSON.stringify());
            break;
        case "browser":
            break;
        default:
...
async function updateTokoSettings()
{
    ...
    await delayedLog();
    $.post('http://wrp-phone/settingsUpdateToko', JSON.stringify({
        tag: "settings",
...
$('#racing-create-form').on('reset', function (e) {
    $.post('http://wrp-phone/racing:map:cancel', JSON.stringify({}));
});

$('#racing-start-tracks').on('change', function (e) {
    let selectedMap = $(this).val();
    if(maps[selectedMap] !== undefined) {
        $.post('http://wrp-phone/racing:map:removeBlips', JSON.stringify({}));
        $.post('http://wrp-phone/racing:map:load', JSON.stringify({ id: selectedMap}));
...
$('#racing-start').submit(function (e) {
    e.preventDefault();
    $.post('http://wrp-phone/racing:event:start', JSON.stringify({
...
$('#racing-create-form').submit(function (e) {
    e.preventDefault();
    $.post('http://wrp-phone/racing:map:save', JSON.stringify({
...
$("#real-estate-sell-form").submit(function (e) {
    e.preventDefault();
    $.post('http://wrp-phone/btnAttemptHouseSale', JSON.stringify({
...
$('#real-estate-transfer-form').submit(function (e) {
    e.preventDefault();
    $.post('http://wrp-phone/btnTransferHouse', JSON.stringify({
...
$("#group-manage-pay-form").submit(function (e) {
    ...
    $.post('http://wrp-phone/payGroup', JSON.stringify({
...
$("#group-manage-rank-form").submit(function (e) {
    e.preventDefault();
    $.post('http://wrp-phone/promoteGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-manage-bank-form").submit(function (e) {
    ...
    $.post('http://wrp-phone/bankGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-tasks-assign-modal-form").submit(function (e) {
    ...
    $.post('http://wrp-phone/btnGiveTaskToPlayer', JSON.stringify({
...
$("#contacts-form").submit(function (e) {
    ...
    $.post('http://wrp-phone/newContactSubmit', JSON.stringify({
        name: clean,
...
$("#stock-form").submit(function (event) {
    event.preventDefault();
    $.post('http://wrp-phone/stocksTradeToPlayer', JSON.stringify({
        identifier: escapeHtml($("#stock-form #stock-id").val()),
        playerid: escapeHtml($("#stock-form #stock-target-id").val()),
        playertarget: escapeHtml($("#stock-form #stock-target-cid").val()),
...
$("#twat-form").submit(function (event) {
    event.preventDefault();
    $.post('http://wrp-phone/newTwatSubmit', JSON.stringify({
...
$("#call-form").submit(function (event) {
    event.preventDefault();
    $.post('http://wrp-phone/callContact', JSON.stringify({
        name: '',
...
$("#new-message-form").submit(function (event) {
    ...
    $.post('http://wrp-phone/newMessageSubmit', JSON.stringify({
    ...
    switch (currentContainer) {
        case "message":
            setTimeout(function () {
                ...
                $.post('http://wrp-phone/messageRead', JSON.stringify({ sender: sender, receiver: receiver, displayName: displayName }));
            }, 300);
            break;
        case "messages":
            setTimeout(function () {
                $.post('http://wrp-phone/messages', JSON.stringify({}));
            }, 300);
            break;
    }
...
$('.racing-map-delete-confirm').click(function () {  
    ...
    $.post('http://wrp-phone/racing:map:delete', JSON.stringify({ id: raceMap }));
    $('.racing-delete-confirm').fadeOut(150)
...
$('#real-estate-evict-modal-accept').click(function () {
    $.post('http://wrp-phone/btnEvictHouse', JSON.stringify({}));
...
$('.twatter-toggle-notification').click(function () {
    ...
    $.post('http://wrp-phone/btnNotifyToggle', JSON.stringify({}));
});

$('.account-information-toggle-pager').click(function () {
    $.post('http://wrp-phone/btnPagerToggle', JSON.stringify({}));
...
$('.racing-entries').on('click', '.racing-entries-join', function () {
    $.post('http://wrp-phone/racing:event:join', JSON.stringify({ identifier: $(this).data('id') }));
});

$('.keys-entries').on('click', '.manage-keys', function () {
    $.post('http://wrp-phone/retrieveHouseKeys', JSON.stringify({}));
});

$('.keys-entries').on('click', '.remove-shared-key', function(e) {
    $.post('http://wrp-phone/removeSharedKey', JSON.stringify({
...
$('.manage-keys-entries').on('click', '.manage-keys-remove', function () {
    $.post('http://wrp-phone/removeHouseKey', JSON.stringify({
...
$('.group-tasks-entries').on('click', '.group-tasks-track', function () {
    $.post('http://wrp-phone/trackTaskLocation', JSON.stringify({ TaskIdentifier: $(this).data('id') }));
});

$('.delivery-job-entries').on('click', '.delivery-job-accept', function (e) {
    $.post('http://wrp-phone/selectedJob', JSON.stringify({ jobType: $(this).data('job-type'), jobId: $(this).data('job-id') }));
});
...
$('.garage-entries').on('click', '.garage-spawn', function (e) {
    e.preventDefault();
    $.post('http://wrp-phone/vehspawn', JSON.stringify({ vehplate: $(this).data('plate') }));
    $.post('http://wrp-phone/btnGarage', JSON.stringify({}));
});

$('.garage-entries').on('click', '.garage-track', function () {
    $.post('http://wrp-phone/vehtrack', JSON.stringify({ vehplate: $(this).data('plate') }));
});

$('.garage-entries').on('click', '.garage-pay', function (e) {
    $.post('http://wrp-phone/vehiclePay', JSON.stringify({ vehiclePlate: $(this).data('plate'), amount: $(this).data('amount') }));
    setTimeout(function () {
        $.post('http://wrp-phone/btnGarage', JSON.stringify({}));
    }, 1500);
});
...
$('.gps-entries, .keys-entries').on('click', '.gps-location-click', function () {
    $.post('http://wrp-phone/loadUserGPS', JSON.stringify({ house_id: $(this).data('house-id'), house_type: $(this).data('house-type') }));
})

$('.contacts-entries, .call-history-entries').on('click', '.btn-contacts-call', function () {
    $.post('http://wrp-phone/callContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
});
...
$('#confirm-modal-accept').click(function (event) {
    $.post('http://wrp-phone/removeContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));

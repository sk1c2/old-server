...
function tryTakeChips(source,amount, chairId, cash)
    ...
    if (chairId == 8 or chairId == 9 or chairId == 10 or chairId == 11 or chairId == 12 or chairId == 13 or chairId == 14) then
        if amount <= 20000 then
            ...
            TriggerClientEvent('DoLongHudText', source, 'Unable to place bet. Amount is too high')
            return false
        end  
    else
...
AddEventHandler("Blackjack:setBlackjackBet",function(gameId,betAmount,chairId, cash)
    local source = source
    ...
    if gameId ~= nil and betAmount ~= nil and chairId ~= nil then 
        ...
        if not blackjackGameInProgress[gameId] then
            if tonumber(betAmount) then
                ...
                if betAmount > 0 then
                    if tryTakeChips(source,betAmount, chairId, cash) then
                        ...
                        TriggerClientEvent("blackjack:notify",source,"Bet placed: " .. tostring(betAmount) .. " chips.")
                    else 
                                ...
                                if blackjackGameData[gameId][source] ~= nil then
                                    ...
                                    if blackjackGameData[gameId][source]["status"] ~= "bust" then 
                                        ...
                                        if currentHand ~= nil then
                                            if currentHand <= 21 then
                                                local potentialWinAmount = math.floor(blackjackGameData[gameId][source][1] * 1.5)
                                                local potentialPushAmount = math.floor(blackjackGameData[gameId][source][1])
                                                ...
                                                if dealerHand > 21 then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("blackjack:notify",source,"You have won $"..tostring(potentialWinAmount))
                                                ...
                                                elseif currentHand == dealerHand then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("blackjack:notify",source,"You have lost: $" ..tostring(potentialPushAmount))
                                                            if potentialPushAmount > 10000000 then

...
local signOnPoint = {
	{x = 441.25, y = -981.97, z = 30.68} -- MRPD
}

RegisterNetEvent('urp:addBlip')
AddEventHandler('urp:addBlip', function(id, first_name)
...
RegisterNetEvent("police:in")
AddEventHandler("police:in" , function(off)
...
RegisterNetEvent('police:openArmoury')
...
RegisterNetEvent('ems:openCabinet')
...
RegisterNetEvent('police:openTrash')
...
RegisterNetEvent('police:openPersonalLocker')
AddEventHandler('police:openPersonalLocker', function()
	local job = exports['isPed']:isPed('job')
	if job == 'Police' then
		local cid = exports['isPed']:isPed('cid')
...
RegisterNetEvent('ems:openPersonalLocker')
AddEventHandler('ems:openPersonalLocker', function()
	local job = exports['isPed']:isPed('job')
	if job == 'EMS' then
		local cid = exports['isPed']:isPed('cid')
...
-- 		if job == "offems" then isMedic = false isCop = false isNews = false end
-- 		if job == "offpolice" then isCop = false isMedic = false isNews = false end
-- 		if job == "news" then isNews = true isMedic = false isCop = false end
-- 		if job == "police" then isCop = true isMedic = false isNews = false end
-- 		if job == "ems" then isMedic = true isCop = false isNews = false end
-- 	end
-- end)
...
local trashlocker = {
	vector3(474.29, -990.72, 26.27), -- mrpd
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if isCop then
			...
			-- Character Switcher
			dstCheck = #(vector3(468.55, -982.01, 30.69) - myCoord)
			if dstCheck < 30 then nearby = true end
			if dstCheck < 3 then
				DrawMarker(25, 468.55, -982.01, 29.75, 0.0, 0.0, 0.0, 0.0, 180.0, 0.0, 1.5, 1.5, 1.5, 0, 0, 255, 100, false, true, 2, nil, nil, false)
				if dstCheck < 1 then
					DrawText3DTest(468.55, -982.01, 30.69,"Press [E] to change characters")
					if IsControlJustReleased(1, 38) then
						TransitionToBlurred(500)
						DoScreenFadeOut(500)
						Wait(1000)
						logout() -- Maybe this works?
						Wait(1000)
					end
				end
			end
	...
	while true do 
		...
		if recentsearch == 0 then
			...
			if endDistance < 0.7 or endDistance2 < 0.7 then
				recentsearch = 1000
				scansuccess = true
				--write script to detect metal here
...
AddEventHandler("isDoctor", function()
	if PlayerData.job.label == "SAMS" then
	isDoctor = true
	end
end)
...
AddEventHandler('police:checkBank', function()
	...
	if(distance ~= -1 and distance < 5) then
		-- print('check bank of target attempt')
...
AddEventHandler('police:checkInventory', function(isFrisk)
		...
		t, distance, closestPed = GetClosestPlayer()
		if(distance ~= -1 and distance < 5) then
			-- print('we must find a way to open targets inventory on that event')
...
AddEventHandler('police:cuff', function(t)
	if not tryingcuff and isCop then
...
AddEventHandler('police:gsr', function(t)
    ...
    if finished == 100 then
		t, distance = GetClosestPlayer()
		if(distance ~= -1 and distance < 7) then
			-- print('there is already available gsr script on fivem forum')
...
AddEventHandler('police:payFines', function(amount)
	...
	TriggerEvent('chatMessagess', 'STATUS: ', {255, 140, 0}, "You were billed for $" .. tonumber(amount) .. " dollar(s).")
end)
...
RegisterNetEvent('add:flag')
AddEventHandler('add:flag', function(licensePlate)
	local myJob = exports["isPed"]:isPed("job")
	if myJob == 'Police' then
      	if licensePlate == nil then
      		TriggerEvent("DoLongHudText", 'Cant target vehicle',2)
		  else
			TriggerEvent('DoLongHudText', 'You have flagged license plate: ' .. licensePlate)
			TriggerServerEvent('flag:plate', licensePlate)
		end
	end
end)

RegisterNetEvent('remove:flag')
AddEventHandler('remove:flag', function(licensePlate)
	local myJob = exports["isPed"]:isPed("job")
	if myJob == 'Police' then
      	if licensePlate == nil then
      		TriggerEvent("DoLongHudText", 'Cant target vehicle',2)
		  else
			TriggerEvent('DoLongHudText', 'You have removed flag off of: ' .. licensePlate)
			TriggerServerEvent('remflag:plate', licensePlate)
		end
	end
end)

RegisterCommand("flagplate", function(source, args)
	local myJob = exports["isPed"]:isPed("job")
	if myJob == 'Police' then
		TriggerEvent('add:flag', args[1])
	else
		TriggerEvent("DoLongHudText", "Required Job: Police")
	end
end)

RegisterCommand("remplate", function(source, args)
	local myJob = exports["isPed"]:isPed("job")
	if myJob == 'Police' then
		TriggerEvent('remove:flag', args[1])
	else
		TriggerEvent("DoLongHudText", "Required Job: Police")
	end
end)
...
AddEventHandler('clientcheckLicensePlate', function()
	if isCop then
      ...
      if licensePlate == nil then
		...
		-- check what of license plate
		TriggerServerEvent('police:check:plate_sv', licensePlate, vehicleClass)
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(2)
		-- Hard cuff
		if IsControlJustPressed(0, 172) and not IsControlPressed(0,19) then
			if exports['isPed']:isPed('job') == 'Police' then
				TriggerEvent('civ:cuffFromMenu', false)
				Citizen.Wait(400)
			end
		end
...
-- 	serviceVehicle(args[1], args[2], false, errorMsg)
-- end)
...
Citizen.CreateThread(function()
	while true do
		...
		local hash21 = GetHashKey(Config.Vehlist[7])
		-- Request models
		RequestModel(hash1)
		RequestModel(hash2)
		RequestModel(hash3)
...
RegisterNetEvent("sv:spawnveh")
AddEventHandler("sv:spawnveh" , function(argh)
	if argh == '1' then
	...
	elseif argh == '14' then
		local model2 = GetHashKey(Config.VehList[14])
		RequestModel(model2)
		local playerPed = PlayerPedId()
		local x,y,z = table.unpack(GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 8.0, 0.5))
		local heading = GetEntityHeading(playerPed)
...
RegisterNetEvent("sv:spawnvehems")
AddEventHandler("sv:spawnvehems" , function(argh)
	if argh == '1' then
		local model2 = GetHashKey(Config.Vehlist[1])
		RequestModel(model2)
		local playerPed = PlayerPedId()
...
AddEventHandler('impoundVehicle', function()
    ...
    targetVehicle = getVehicleInDirection(coordA, coordB)
	licensePlate = GetVehicleNumberPlateText(targetVehicle)
	TriggerServerEvent("garages:SetVehImpounded", licensePlate)
	deleteVeh(targetVehicle)
end)
...
AddEventHandler('fullimpoundVehicle', function()
    ...
    coordB = GetOffsetFromEntityInWorldCoords(playerped, 0.0, 100.0, 0.0)
   	targetVehicle = getVehicleInDirection(coordA, coordB)
	licensePlate = GetVehicleNumberPlateText(targetVehicle)
	TriggerServerEvent("garages:SetVehImpounded2",licensePlate)
	deleteVeh(targetVehicle)
end)

RegisterCommand('impound', function(source)
	local job = exports['isPed']:isPed('job')
	if job == 'Police' or job == 'tuner_carshop' or job == 'tuner_shop' or job == 'EMS' or job == 'CamelTowing' then
		TriggerEvent('impoundVehicle', source)
	elseif job == 'EMS' then
		TriggerServerEvent("impoundVehicle")
	end
end)

RegisterNetEvent('police:impoundmenu')
AddEventHandler('police:impoundmenu', function()
	playerped = PlayerPedId()
    coordA = GetEntityCoords(playerped, 1)
    coordB = GetOffsetFromEntityInWorldCoords(playerped, 0.0, 100.0, 0.0)
    targetVehicle = getVehicleInDirection(coordA, coordB)
...
RegisterNetEvent('carryfunction', function()
	if not carry.InProgress then
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				TriggerEvent('DoLongHudText', "No one nearby to carry!")
			end
		else
			TriggerEvent('DoLongHudText', "No one nearby to carry!")
		end

...
function Save(save)
    if save then
        data = GetCurrentPed()
        TriggerServerEvent("raid_clothes:insert_character_current", data, exports['isPed']:isPed('cid'))
        if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
            TriggerServerEvent("raid_clothes:insert_character_face", data, exports['isPed']:isPed('cid'))
            TriggerServerEvent("raid_clothes:set_tats", currentTats, exports['isPed']:isPed('cid'))
        end
    else
        LoadPed(oldPed)
    end
...
Citizen.CreateThread(function()
	...
	while true do
        ...
        if nearcloth < 3.0 and jailcheck == 512001 then
            sleepthread = 1
            StoreCost = 0;
            menu = {"clothesmenu", "Press ~g~M~s~ to change in to prison outfits"}
        elseif nearcloth < 3.0 then
            sleepthread = 1
...
RegisterNetEvent('raid_clothes:missingcharID')
AddEventHandler('raid_clothes:missingcharID', function()
    print('No Character ID Found')
    print('Going ahead with exporting CID')
    TriggerServerEvent('raid_clothes:get_character_face', exports['isPed']:isPed('cid'))
end)
...
RegisterCommand("outfits", function(source, args, rawCommand)
    ...
    local distance = GetDistanceBetweenCoords(plyCoords.x,plyCoords.y,plyCoords.z,209.9024, -902.5281, -98.07497,false)
    if (IsNearShop(clothingShops) < 5.0) then

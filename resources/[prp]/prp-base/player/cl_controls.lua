...
URP.Controls.secondaryBinds = {}

URP.Controls.Distcheck = {}
URP.Controls.Distcheck.use = {
	-- create fake id
...
URP.Controls.Distcheck.useSecondary = {
...
URP.Controls.Distcheck.useThird = {
	-- hospital garage
...
-- function URP.DataControls.encodeSetBindTable(self, bindTable)
...
-- 	URP.DataControls.setBindTable(controlTable,true)
-- end

-- function URP.DataControls.toUpper(table)
...
-- function URP.DataControls.setBindTable(controlTable,shouldSend)
...
-- 		end
-- 	end
-- 	TriggerEvent("event:control:update",URP.DataControls.getTableInKeyNumbers())
-- end

-- function URP.DataControls.setSecondaryBindTable(bindtable)
...
-- 			local valid = true
-- 			for i=1,#URP.Controls.secondaryBinds  do
-- 				if URP.Controls.secondaryBinds[i] and URP.Controls.secondaryBinds[i][1] == control1 then valid = false end
-- 			end

-- 			if valid then 
-- 				URP.Controls.secondaryBinds[i] = {}
-- 				URP.Controls.secondaryBinds[i][1] = control1
-- 				URP.Controls.secondaryBinds[i][2] = control2
-- 				i = i + 1
-- 			end
...
-- function URP.DataControls.getTableInKeyNumbers()
...
-- function URP.DataControls.getBindByID(bindID)
-- 	local found = "none"
-- 	for k,v in pairs(URP.Controls.Current) do
-- 		if k == bindID then
-- 			found = v
...
-- function URP.DataControls.distanceCall(distcheckName)
-- 	local distanceTable = URP.Controls.Distcheck[distcheckName]
-- 	local found = -1
...
-- 		if URP.Controls.EventHolder[key] then
-- 			hasTimer = (GetGameTimer()-URP.Controls.EventHolder[key])
-- 		end
-- 		if  hasTimer >= distanceTable[found].maxwait then
-- 			URP.Controls.EventHolder[key] = GetGameTimer();
...
-- function URP.DataControls.checkForMissing()
-- 	local isMissing = false

-- 	for j,h in pairs(URP.Controls.Default) do
-- 		if URP.Controls.Current[j] == nil then
-- 			isMissing = true
-- 			URP.Controls.Current[j] = h
-- 		end
-- 	end
...
-- function URP.DataControls.validEvent(event,id,eventID)
-- 	local v = URP.Controls.events["general"][eventID]
-- 	local hasTimer = 999999
-- 	local eventKey = event.."_"..id

-- 	if URP.Controls.EventHolder[eventKey] then
-- 		hasTimer = (GetGameTimer()-URP.Controls.EventHolder[eventKey])
-- 	end

-- 	if URP.Controls.EventHolder[eventKey] and hasTimer ~= 999999 then
-- 		if hasTimer >= v.maxwait then
-- 			URP.Controls.EventHolder[eventKey] = GetGameTimer();
...
-- 		if URP.Controls.Current["tokoptt"] then
-- 			for i=0,#URP.Controls.events["general"] do
-- 				local v = URP.Controls.events["general"][i]
-- 				local keyString = URP.Controls.Current[v["bind"]]
-- 				local key = Keys[keyString]

-- 				if key == 199 then
-- 					if IsDisabledControlJustReleased(1,key) then
-- 						local isConflicted = -1
-- 						for i=0,#URP.Controls.secondaryBinds do
-- 							if IsControlPressed(1,URP.Controls.secondaryBinds[i][1]) or IsDisabledControlPressed(1,URP.Controls.secondaryBinds[i][1]) then
-- 								isConflicted = URP.Controls.secondaryBinds[i][1]
-- 							end
-- 						end
...
-- 								URP.DataControls.validEvent(eventBind,idEvent,i)
-- 							else
-- 								URP.DataControls.validEvent(v.event,v.id,i)
-- 							end
-- 						else
...
-- 								local isConflicted = -1
-- 								for i=0,#URP.Controls.secondaryBinds do
-- 									if IsControlPressed(1,URP.Controls.secondaryBinds[i][2]) or IsDisabledControlPressed(1,URP.Controls.secondaryBinds[i][2]) then
-- 										isConflicted = URP.Controls.secondaryBinds[i][2]
-- 									end
-- 								end
...
-- 		if URP.Controls.Current["tokoptt"] then
-- 			for i=0,#URP.Controls.events["general-dist"] do
-- 				local v = URP.Controls.events["general-dist"][i]
-- 				local key = Keys[URP.Controls.Current[v["bind"]]]
-- 				local hasTimer = 99999;
...
-- 					URP.DataControls.distanceCall(v["distanceName"])
-- 				end	
-- 			end
-- 		end
-- 		Wait(5)

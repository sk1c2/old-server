...
$(document).ready(function () {
    $('.save-settings').click(() => {
        ...
        $.post(
            'https://wrp-inventory/UpdateSettings',
            JSON.stringify({
...
function invStack(
    ...
    $.post('https://wrp-inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://wrp-inventory/move', JSON.stringify(arr));
}
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory, itemid1, metainformation1, itemid2, metainformation2) {
    ...
    $.post('https://wrp-inventory/swap', JSON.stringify(arr));
}

function removeCraftItems(itemid, moveAmount) {
    let arr = itemList[itemid].craft;
    let amount = moveAmount;
    $.post('https://wrp-inventory/removeCraftItems', JSON.stringify([arr, amount]));
}
...
function DisplayInventoryMultiple(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, Owner, targetInvWeight = 0) {
    ...
    } else if (targetinvName.indexOf('whitewidow_counter') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 10;
        displayName = 'Pickup Order';
    } else if (targetinvName.indexOf('gem_counter') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 10;
        displayName = 'Gem Table';
    } else if (targetinvName.indexOf('bakery_counter') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 10;
        displayName = 'Bakery Pickup Order';
    } else if (targetinvName.indexOf('bakery_shelf') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 10;
        displayName = 'Bakery Shelf';
    } else if (targetinvName.indexOf('bakery_drinks') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 10;
        displayName = 'Bakery Drink Shelf';
    } else if (targetinvName.indexOf('bakery_storage') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 20;
        displayName = 'Bakery Storage';
    }else {
        secondaryMaxWeight = 250.0;
        slotLimitTarget = 40;
    }
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://wrp-inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
    ...
    for (i = 0; i < slotLimit + 1; i++) {
        if (isEmpty(inventory[i])) {
            ...
            if (itemList[itemid] !== undefined) {
                ...
                $.post("http://wrp-inventory/updateMyQuality", JSON.stringify({
                    itemid: itemid,
                    slot: slot,
                    quality: quality
                }));
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            $.post('https://wrp-inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function DropItem(slot, amountDropped) {
    ...
    // $.post('https://wrp-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
    // $.post('https://wrp-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://wrp-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    $.post(
        'https://wrp-inventory/SlotJustUsed',
        JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {
    // $.post('https://wrp-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post(
        'https://wrp-inventory/ServerCloseInventory',
        JSON.stringify({
...
function useitem() {
    ...
    if (inventoryUsedName == PlayerInventoryName) {
        ...
        $.post('https://wrp-inventory/invuse', JSON.stringify(arr));

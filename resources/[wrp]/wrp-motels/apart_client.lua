			...
			if entryType == 1 then
				...
				if myhouseid ~= 0 then
					...
					elseif mansion2 then
						...
						if (#(vector3(curHouseCoords["x"]-10,curHouseCoords["y"]+5.7771,curHouseCoords["z"]-16) - playerCoords) < 5.2 ) then
							DrawText3Ds(curHouseCoords["x"]-10,curHouseCoords["y"]+5.7771,curHouseCoords["z"]-16, '~g~'..Controlkey["housingMain"][2]..'~s~ front door' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								TriggerServerEvent("houserobberies:exit",myhouseid,mansion,false)
								Citizen.Wait(5000)
							end
...
function buildFranklin()
	...
	local generator = { x = curHouseCoords["x"] , y = curHouseCoords["y"], z = curHouseCoords["z"] - 28.0}
	SetEntityCoords(PlayerPedId(), generator.x, generator.y,generator.z+7)



	local building = CreateObject(`shell_michael`,generator.x,generator.y,generator.z,false,false,false)
	FreezeEntityPosition(building,true)
	SetEntityCoords(PlayerPedId(), generator.x-10, generator.y+5.7771,generator.z+12)

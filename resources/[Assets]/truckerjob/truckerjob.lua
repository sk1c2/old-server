...
--            TriggerServerEvent("server:givepayJob", "Delivery Service", payment)
            TriggerServerEvent( 'server:givepayJob', 250 )
          end
          Wait(1)
        end
...
AddEventHandler("Trucker:SelectJob", function(data)
    ...
    job = currentJobs[math.random(#currentJobs)].id
    clearBlip()
    currentJobPos = returnJobPos(job)
...
RegisterCommand("trucker", function()
  TriggerEvent("Trucker:SelectJob")
end)
    ...
    while not hasPackedCargo do
        ...
        if( #(playerCo - vector3( back["x"], back["y"], back["z"] )) < 15.0) then
            DrawText3Ds(back["x"], back["y"], back["z"],"Press ~r~"..Controlkey["generalUse"][2].."~w~ to store then check for more.")
              print(#(playerCo - vector3( back["x"], back["y"], back["z"] )))
            if( IsControlJustPressed(1, 46) ) and #(playerCo - vector3( back["x"], back["y"], back["z"] )) < 1.5 then
              print(3111)
                stopCarry()
                attachBox()
                hasPackedCargo = true

...
AddEventHandler('dispatch:clNotify', function(pNotificationData)
  if pNotificationData.dispatchMessage and pNotificationData.isImportant ~= nil then
...
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
        if (IsControlJustReleased(0, 47) and (exports['isPed']:isPed('job') == 'EMS' or exports['isPed']:isPed('job') == 'Police')) then
...
function AlertOfficerDown(object)
  local street1 = GetStreetAndZone()
  local gender = IsPedMale(PlayerPedId())
  local targetVehicle = object
  local origin = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent('meth:alertPD')
AddEventHandler('meth:alertPD', function()
  MethAlert()
end)

function MethAlert(object)
  local street1 = GetStreetAndZone()
  local gender = IsPedMale(PlayerPedId())
  local targetVehicle = object
  local origin = GetEntityCoords(PlayerPedId())
  if not DoesEntityExist(targetVehicle) then
    ...
    TriggerServerEvent('dispatch:svNotify', {
      ...
      isImportant = true,
      priority = 1,
      dispatchMessage = "Suspicious Vehicle (Strong Odors)",
      origin = {
        x = plyPos.x,
        y = plyPos.y,
        z = plyPos.z
...
function AlertPowerPlant()
  ...
  TriggerServerEvent('dispatch:svNotify', {
    ...
    isImportant = true,
    priority = 1,
    dispatchMessage = "Hitting Power Plant",
    origin = {
      x = plyPos.x,
      y = plyPos.y,
      z = plyPos.z
...
function AlertDrugJob()
  ...
  TriggerServerEvent('dispatch:svNotify', {
    ...
    isImportant = true,
    priority = 1,
    dispatchMessage = "Drug Selling in Progress",
    origin = {
      x = plyPos.x,
      y = plyPos.y,
      z = plyPos.z
...
RegisterNetEvent('urp:alert:houserobbery')
AddEventHandler("urp:alert:houserobbery",function()
  AlertCheckRobbery2()
end)

RegisterNetEvent('urp:alert:downplayer')
AddEventHandler('urp:alert:downplayer', function()
  AlertDeath()
end)

RegisterNetEvent('urp:alert:jailbreak')
AddEventHandler('urp:alert:jailbreak', function()
  AlertJailBreak()
end)

RegisterNetEvent('urp:alert:bigbank')
AddEventHandler('urp:alert:bigbank', function()
  AlertBigBankRobbery()
end)

RegisterNetEvent('urp:alert:powerplant')
AddEventHandler('urp:alert:powerplant', function()
  AlertPowerPlant()
end)


RegisterNetEvent('urp:alert:fleeca')
AddEventHandler('urp:alert:fleeca', function()
  AlertFleecaRobbery()
end)

RegisterNetEvent('urp:alert:storerobbery')
AddEventHandler('urp:alert:storerobbery', function()
  AlertpersonRobbed()
end)

RegisterNetEvent('urp:alert:drugjob')
AddEventHandler('urp:alert:drugjob', function()
  AlertDrugJob()
end)

RegisterNetEvent('urp:alert:vehtheft')
AddEventHandler('urp:alert:vehtheft', function()
  AlertCheckLockpick()
end)


RegisterCommand('togglealerts', function(args, user)
  if exports['isPed']:isPed('job') == 'Police' or exports['isPed']:isPed('job') == 'EMS' then

...
function runRecycle()
    ...
    FreezeEntityPosition(GetPlayerPed(-1),false)
    	...
    	TriggerEvent('wrp-banned:getID',"recyclablematerial", math.random(3,18))
...
listOn = false
	while true do
		...
		if GetDistanceBetweenCoords(PlayerPos, 5.87, -1601.48, 29.3, true) <= 1.5 then	
		...
		--	end
			...
			if GetDistanceBetweenCoords(PlayerPos, 2221.7973632813,5577.0966796875,53.844783782959, true) <= 10 then		
				...
				if JustDriedWeed or JustPackedWeed then
					...
					TriggerEvent("wrp-banned:getID","wetbud",math.random(3))
				end
			end		
			-- HARVEST WEED - END			
			...
			if GetDistanceBetweenCoords(PlayerPos, -1171.6, -1572.13, 4.67, true) <= 1 then						
				-- selling bud
				-- local smallbud = exports["wrp-inventory"]:hasEnoughOfItem("smallbud",5,false)
				if exports["wrp-inventory"]:hasEnoughOfItem("smallbud",5) then
					TriggerEvent("inventory:removeItem", "smallbud", 5)
					TriggerServerEvent('weed:sell', math.random(50, 125) )
				else
			...
			if GetDistanceBetweenCoords(PlayerPos, -2079.83, 2610.12, 3.08, true) <= 5 then						
				...
				if JustPickedWeed or JustPackedWeed then
					...
					local wetbud = exports["wrp-inventory"]:hasEnoughOfItem("wetbud",5,false)
					if wetbud then

						TriggerEvent("inventory:removeItem", "wetbud", 5)
						local finished = exports["wrp-taskbar"]:taskBar(10000,"Drying Weed",false,false,playerVeh)
						if (finished == 100) then
							TriggerEvent("wrp-banned:getID","smallbud",math.random(10))
						end
					else
			...
			-- if GetDistanceBetweenCoords(PlayerPos, , true) <= 5 then						
			...
			-- 			JustPackedWeed = true
			-- 		end
			-- 		local wetbud = exports["wrp-inventory"]:hasEnoughOfItem("smallbud",5,false)
			-- 		if wetbud then

			-- 			local finished = exports["wrp-taskbar"]:taskBar(10000,"Packing Weed",false,false,playerVeh)
			-- 			if (finished == 100) then
			-- 				TriggerEvent("inventory:removeItem", "smallbud", 5)
			-- 				TriggerEvent("wrp-banned:getID","smallbud",math.random(10))
			-- 			end
			...
			if GetDistanceBetweenCoords(PlayerPos, 5.87, -1601.48, 29.3, true) <= 5 then						
				...
				local smallbud = exports["wrp-inventory"]:hasEnoughOfItem("freshmeat",5,false)
				if smallbud then
			...
			if GetDistanceBetweenCoords(PlayerPos, 994.38,-3099.98,-38.99, true) <= 3 and not isTrading then
				TriggerEvent("wrp-ac:triggeredItemSpawn", "141", "Craft"); 
			end
			Wait(2500)
		end
	end
			...
			if itemRandom < 550 then
				if itemRandom < 30 then
					...
					for i,v in ipairs(materialsTable) do
						...
						TriggerEvent('wrp-banned:getID',recyclablematerial, rnd)
					end
				...
				TriggerEvent("wrp-banned:getID","freshmeat",math.random(3))
			end
...
Citizen.CreateThread(function()
	while true do
		...
		local drawtext = "~b~ E ~w~- To enter Recycling Plant"
		local plyCoords = GetEntityCoords(Getmecuh)
		local x,y,z = 746.76, -1399.39,26.62
		...
		if distance <= 1.2 then
			...
			if IsControlJustReleased(0, 38) then
				...
				DoScreenFadeOut(400)
				Citizen.Wait(500)
				SetEntityCoords(Getmecuh, 997.95, -3092.01,-39.0)
				Citizen.Wait(250)
				DoScreenFadeIn(250)
			end
		else
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(5)
		local Getmecuh = PlayerPedId()
		local drawtext = "~b~ E ~w~- To leave Recycling Plant"
		local plyCoords = GetEntityCoords(Getmecuh)
		local x,y,z = 997.95, -3092.01,-39.0
		local distance = GetDistanceBetweenCoords(plyCoords.x,plyCoords.y,plyCoords.z,x,y,z ,false)
		if distance <= 1.2 then
			DrawText3Ds(x,y,z, drawtext) 
			if IsControlJustReleased(0, 38) then
				...
				DoScreenFadeOut(400)
				Citizen.Wait(500)
				SetEntityCoords(Getmecuh, 746.76, -1399.39,26.62)
				Citizen.Wait(250)
				DoScreenFadeIn(250)
			end
		else

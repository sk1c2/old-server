...
function CloseCreator(name, boat, price)
	Citizen.CreateThread(function()
		...
		if not boughtboat then
			...
			TriggerServerEvent('ply_docks:BuyForBoat', name, boat, price, plate, primarycolor, secondarycolor, pearlescentcolor, wheelcolor, cid)
...
function ButtonSelected(button)
	...
	elseif this == "Pneumatique" or this == "Voiles" or this == "Jetski" or this == "Chalutier" or this == "Boats" then
		TriggerEvent('ply_docks:CheckMoneyForBoat',button.name,button.model,button.costs)
	end
end
...
AddEventHandler('ply_docks:CheckMoneyForBoat', function(name, boat, price)
    ...
    if Player.cash >= tonumber(price) then
      ...
      TriggerEvent('ply_docks:BuyFalseTrue', source)
    end
end)

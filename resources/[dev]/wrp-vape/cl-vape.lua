	...
	if not exports["wrp-deathmanager"]:GetDeath() then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToVape == false then
				...
				TriggerEvent('DoLongHudText', 'You have started using your vape.', 1)
				Wait(Config.HelpTextLength)
				TriggerEvent("Vape:HelpFadeOut", 0)
			else
				TriggerEvent('DoLongHudText', 'You are already holding your vape.', 2)
			end
		else
			TriggerEvent('DoLongHudText', 'You can not do this in a vehicle.', 2)
		end
	else
		TriggerEvent('DoLongHudText', 'You can not do this if you are dead.', 2)
	end
end)
...
AddEventHandler("Vape:StopVaping", function(source)
	if IsPlayerAbleToVape == true then
		...
		TriggerEvent('DoLongHudText', 'You have stopped using your vape.', 2)
	end
end)
AddEventHandler("Vape:Drag", function()
	if IsPlayerAbleToVape then
		...
		if (DoesEntityExist(ped) and not IsEntityDead(ped)) then
			...
			local VapeCoil = math.random(1,Config.VapeCoil)
			if VapeCoil == 1 then
				TaskPlayAnim(ped, ad, anim, 8.00, -8.00, -1, (2 + 16 + 32), 0.00, 0, 0, 0)
		...
		TriggerEvent('DoLongHudText', 'You must be holding your vape to do this.', 1)
	end
end)
...
if Config.Debug then
	RegisterCommand("vapesound", function(source, rawCommand)
		...
		TriggerEvent('DoLongHudText', 'Play sound???', 2)
	end)
	RegisterCommand("vapexp", function(source, rawCommand)
		local ped = GetPlayerPed(-1)
		local PedPos = GetEntityCoords(ped)
		AddExplosion(PedPos.x, PedPos.y, PedPos.z+1.00, 34, 0.00, true, false, 1.00)
		TriggerEvent('DoLongHudText', 'Play explosion???', 2)
	end)
end

RegisterCommand('stop', function (source)

...
local anni_hash = GetHashKey("annihilator")
local ems_hash = GetHashKey("emsair")
...
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(5)
		if IsPlayerInPolmav() or IsPlayerInAnni() or IsPlayerInEMS() then
		...
		if IsPlayerInPolmav() or IsPlayerInAnni() or IsPlayerInEMS() and target_vehicle and not helicam and vehicle_display ~=2 then
...
function IsPlayerInAnni()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, anni_hash)
end

function IsPlayerInEMS()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)

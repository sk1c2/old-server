...
function OpenCheck(curClosestNum)
  ...
  local job = exports['isPed']:isPed('job')
    ...
    if (job == "Police") and (curClosestNum >= 1 and curClosestNum <= 40) then
        return true
    end

    if (job == "Police") and (curClosestNum >= 137 and curClosestNum <= 140) then
        return true
    end

    if (job == "Police" or job == "DOC") and (curClosestNum >= 40 and curClosestNum <= 98) then
        return true
    end
    
    if (job == "EMS" or job == "Police") and (curClosestNum >= 18 and curClosestNum <= 32) then
        return true
    end
    ...
    if (job == "tuner_carshop" or job == "tuner_shop") and (curClosestNum >= 97 and curClosestNum <= 100) then
        return true
    end

    if (job == "DriftSchool") and (curClosestNum >= 105 and curClosestNum <= 109) then
        return true
    end

    if (job == "VanillaUnicorn" or job == "Police") and (curClosestNum >= 100 and curClosestNum <= 104) then
        return true
    end

    if (job == "FIB") and (curClosestNum >= 109 and curClosestNum <= 111) then
        return true
    end

    if (job == "Gallery") and (curClosestNum >= 111 and curClosestNum <= 130) then
        return true
    end

    if (job == "WhiteWidow") and (curClosestNum >= 130 and curClosestNum <= 133) then
        return true
    end

    if (job == "BurgerShot") and (curClosestNum >= 134 and curClosestNum <= 138) then
        return true
    end
end

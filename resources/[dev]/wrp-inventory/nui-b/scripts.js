...
$(document).ready(function () {
    ...
    window.addEventListener('message', function (event) {
        ...
        if (item.response == 'openGui') {
            ...
            $('#containers-wrapper').slideUp(400);
            $('#app').fadeOut(400);
            $('#containers-wrapper').fadeOut(400);
...
function invStack(
    ...
    $.post('https://bhrp-inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://bhrp-inventory/move', JSON.stringify(arr));
}
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory, itemid1, metainformation1, itemid2, metainformation2) {
    ...
    $.post('https://bhrp-inventory/swap', JSON.stringify(arr));
}

function removeCraftItems(itemid, moveAmount) {
    let arr = itemList[itemid].craft;
    let amount = moveAmount;
    $.post('https://bhrp-inventory/removeCraftItems', JSON.stringify([arr, amount]));
}
...
function DisplayInventoryMultiple(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, Owner, targetInvWeight = 0) {
    ...
    if (targetinvName.indexOf("Drop") > -1) {
        ...
        slotLimitTarget = 5;
    } else if (targetinvName.indexOf("Counter") > -1) {
        secondaryMaxWeight = 50.0
        slotLimitTarget = 5;
    } else if (targetinvName.indexOf("McD") > -1) {
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://bhrp-inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            $.post('https://bhrp-inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function DropItem(slot, amountDropped) {
    ...
    // $.post('https://bhrp-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
    // $.post('https://bhrp-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://bhrp-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    $.post(
        'https://bhrp-inventory/SlotJustUsed',
        JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {
    // $.post('https://bhrp-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post(
        'https://bhrp-inventory/ServerCloseInventory',
        JSON.stringify({
...
function useitem() {
    ...
    if (inventoryUsedName == PlayerInventoryName) {
        let arr = [inventoryUsedName, itemid, slotusing, isWeapon, durability];
        $.post('https://bhrp-inventory/invuse', JSON.stringify(arr));

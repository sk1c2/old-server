...
local cashPayment = math.random(100, 1000)
...
local OxyDropOffs = {
	[1] =  { ['x'] = 131.9016,['y'] = -1490.293,['z'] = 29.14038,['h'] = 320.61221313477, ['info'] = ' 1' },
	[2] =  { ['x'] = -35.96688,['y'] = -1415.476,['z'] = 29.31215,['h'] = 179.33856201172, ['info'] = ' 2' },
	[3] =  { ['x'] = -196.2854,['y'] = -1555.618,['z'] = 38.33495,['h'] = 230.0777130127, ['info'] = ' 3' },
	[4] =  { ['x'] = 1042.103,['y'] = -2417.357,['z'] = 29.2953,['h'] = 84.02713775, ['info'] = ' 4' },
...
AddEventHandler("rolexdelivery:client", function()
	...
	while tasking do
		...
		if toolong < 0 then
			...
			tasking = false
			OxyRun = false
			TriggerEvent("DoLongHudText","You are no longer selling Rolexs due to taking too long to drop off.")
		end
...
RegisterNetEvent("rolexdrugdelivery:client")
AddEventHandler("rolexdrugdelivery:client", function()
...
Citizen.CreateThread(function()
    ...
    while true do
	  ...
	  --  local dropOff3 = #(vector3(GetEntityCoords(PlayerPedId())) - vector3(chopinfo[3]["x"],chopinfo[3]["y"],chopinfo[3]["z"]))
	    ...
	    local dropOff6 = #(GetEntityCoords(PlayerPedId()) - vector3(182.2871, -1319.154, 29.31593))
            ...
            if IsControlJustReleased(0,38) then
                ...
                if Player.cash >= costs then
					if exports["prp-inventory"]:hasEnoughOfItem("rolexwatch",5,true) then
						...
						TriggerEvent('DoLongHudText', 'You do not have enough rolex\'s!', 2)
					end
				else
					TriggerEvent('DoLongHudText', 'You do not have enough money to start!', 2)
				end
                --TriggerServerEvent("rolexdelivery:server",1500,"robbery",50)
                Citizen.Wait(1000)
            end
	    ...
	    if dropOff2 < 20.0 then
	    	...
	    	if dropOff2 > 2.0 and dropOff5 > 2.0 and dropOff6 > 2.0 then
		    	Citizen.Wait(1000)
		    end
...
Citizen.CreateThread(function()
    while true do
        if drugdealer then
	        ...
	        TriggerEvent("rolexdrugdelivery:client")  

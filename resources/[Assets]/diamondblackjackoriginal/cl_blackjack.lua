...
local hasHit = false
local hasStood = false
local hasLeft = false
local hasSetHand = false
local LocalPlayer
local Player
    ...
    while true do 
        if closeToCasino then
            if waitingForBetState then
                if betSubmitted then --Submit bet [Enter]
                    if tonumber(currentBetAmount) >= 0 then
                        TriggerServerEvent("Blackjack:setBlackjackBet",globalGameId,currentBetAmount,closestChair, exports['isPed']:isPed('cash'))
    ...
    while true do
        if closeToCasino then
            if sittingAtBlackjackTable and canExitBlackjack then
                ...
                if hasLeft and not waitingForSitDownState then  --Leave seat [backspace]
...
RegisterNUICallback('setbet', function(data, cb)
    ...
    SendNUIMessage({
        type = 'closeBet',
        status = '$' .. currentBetAmount .. ' placed. Waiting for other players...',
        cash = Player.cash
    })
    SetNuiFocus(false, false)
...
RegisterNUICallback('hitorstandstate', function(data, cb)
    if data.hasHit ~= nil and waitingForPlayerToHitOrStand then
        hasHit = data.hasHit
    end

    if data.hasStood ~= nil and waitingForPlayerToHitOrStand then

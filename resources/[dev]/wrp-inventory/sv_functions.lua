Citizen.CreateThread(function()
    Citizen.Wait(2000)
    print('[wrp-inventory]: Authorized.')
end)
...
RegisterServerEvent('wrp-inventory:openInventorySteal')
AddEventHandler('wrp-inventory:openInventorySteal', function(target)
    local src = source
    -- print(source)
    if target ~= nil then
        ...
        TriggerClientEvent('wrp-inventory:openInventorySteal', src, char)
        ...
        exports.ghmattimysql:execute('SELECT `cash` FROM __characters WHERE id = ?', {char.id}, function(data)
            TriggerClientEvent('wrp-ac:removeban', target, data[1].cash)
            Citizen.Wait(100)
            TriggerClientEvent('wrp-ac:InfoPass', src, data[1].cash)
        end)
    end
end)

RegisterServerEvent('wrp-inventory:openInventorySteal2')
AddEventHandler('wrp-inventory:openInventorySteal2', function(target)
    local source = source
    -- print(source)
    if target ~= nil then
        -- print(target)
...
RegisterServerEvent('wrp-inventory:openInventorySteal3')
AddEventHandler('wrp-inventory:openInventorySteal3', function(target)
    local src = source
    -- print(source)
    if target ~= nil then
        -- print(target)
...
AddEventHandler('loot:useItem', function(itemid)
    TriggerClientEvent('wrp-banned:getID', source, '-771403250', math.random(6,8))
end)

RegisterServerEvent('loot:redpack')
AddEventHandler('loot:redpack', function(itemid)
    TriggerClientEvent('wrp-banned:getID', source, 'ciggy', 10)
end)
...
RegisterServerEvent('wrp-inventory:logInventoryOpen')
AddEventHandler('wrp-inventory:logInventoryOpen', function(cid, target, name)
...
RegisterServerEvent('wrp-inventoryLog')
AddEventHandler('wrp-inventoryLog', function(cash)
...
RegisterServerEvent('wrp-DeleteGunLog')
AddEventHandler('wrp-DeleteGunLog', function(cash)
...
RegisterServerEvent('wrp-NoReloadLog')
AddEventHandler('wrp-NoReloadLog', function(cash)
...
RegisterServerEvent('wrp-InfLog')
AddEventHandler('wrp-InfLog', function(cash)
...
RegisterServerEvent('wrp-speclog')
AddEventHandler('wrp-speclog', function(cash)
...
RegisterServerEvent('wrp-swatlog')
AddEventHandler('wrp-swatlog', function(cash)
...
RegisterServerEvent('wrp-vehspeedboost')
AddEventHandler('wrp-vehspeedboost', function(cash)
...
RegisterServerEvent('wrp-explosivepunch')
AddEventHandler('wrp-explosivepunch', function(cash)
...
RegisterServerEvent('wrp-weed:Use-Water')
AddEventHandler('wrp-weed:Use-Water', function(amount)
    sendToDiscord6("Plant Water", "**" .. GetPlayerName(source) .. "** has gave a plant water. \n\n**Player : **" .. GetPlayerName(source), 65280)
end)

RegisterServerEvent('wrp-weed:Use-Fertiliser')
AddEventHandler('wrp-weed:Use-Fertiliser', function(amount)
    sendToDiscord6("Plant Fertiliser", "**" .. GetPlayerName(source) .. "** has gave a plant fertiliser. \n\n**Player : **" .. GetPlayerName(source), 65280)
end)

RegisterServerEvent('wrp-meth:pass')
AddEventHandler('wrp-meth:pass', function(cash)
...
RegisterServerEvent('wrp-meth:pseudo')
AddEventHandler('wrp-meth:pseudo', function(amount)
    sendToDiscord3("Dumped Pseudophedrine", "**" .. GetPlayerName(source) .. "** Dumped Pseudophedrine. \n\n**Dumped Pseudophedrine Amount: **" .. amount, 65280)
end)

RegisterServerEvent('wrp-meth:hintObtain')
AddEventHandler('wrp-meth:hintObtain', function(amount)
...
RegisterServerEvent('wrp-business:log')
AddEventHandler('wrp-business:log', function(amount, total, job)
    sendToDiscord7('Business Name: ' ..job.. ' \nGained: $' ..amount.. " and now have a total of: $" ..total)
end)

RegisterServerEvent('wrp-business:logr')
AddEventHandler('wrp-business:logr', function(amount, total, job)
    sendToDiscord7('Business Name: ' ..job.. ' \nLost: $' ..amount.. " and now have a total of: $" ..total)
end) -- sendToDiscord8

RegisterServerEvent('wrp-mine:autoclicker')
AddEventHandler('wrp-mine:autoclicker', function()
    local src = source
...
RegisterServerEvent('wrp-pedLog:logAllInfo')
AddEventHandler('wrp-pedLog:logAllInfo', function(fullname, cid, model, hairColor, headBlend, headOverlay, headStructure)
    local src = source

...
AddEventHandler("raid_clothes:insert_character_current",function(data, cid)
...
AddEventHandler("raid_clothes:insert_character_face",function(data, cid)
    if not data then return end
    local src = source
...
AddEventHandler("raid_clothes:get_character_face",function(cid)
    local src = source
    local characterId = cid
...
AddEventHandler("raid_clothes:get_character_current",function(cid)
    local src = source
    local characterId = cid

    if not characterId then return end

    exports.ghmattimysql:execute("SELECT `model`, `drawables`, `props`, `drawtextures`, `proptextures` FROM character_current WHERE cid = @cid", {['cid'] = characterId}, function(result)
        local temp_data = {
...
AddEventHandler("raid_clothes:retrieve_tats", function(cid)
    local src = source
	exports.ghmattimysql:execute("SELECT `tattoos` FROM playersTattoos WHERE identifier = @identifier", {['identifier'] = cid}, function(result)
        if(#result == 1) then
			TriggerClientEvent("raid_clothes:settattoos", src, json.decode(result[1].tattoos))
		else
			local tattooValue = "{}"
			exports.ghmattimysql:execute("INSERT INTO playersTattoos (identifier, tattoos) VALUES (@identifier, @tattoo)", {['identifier'] = cid, ['tattoo'] = tattooValue})
...
AddEventHandler("raid_clothes:set_tats", function(tattoosList, cid)
	local src = source
	exports.ghmattimysql:execute("UPDATE playersTattoos SET tattoos = @tattoos WHERE identifier = @identifier", {['tattoos'] = json.encode(tattoosList), ['identifier'] = cid})
end)


RegisterServerEvent("raid_clothes:get_outfit")
AddEventHandler("raid_clothes:get_outfit",function(slot, cid)
    ...
    exports.ghmattimysql:execute("SELECT `model`, `drawables`, `props`, `drawtextures`, `proptextures`, `hairColor` FROM character_outfits WHERE cid = @cid and slot = @slot", {
        ['cid'] = characterId,
        ['slot'] = slot
    }, function(result)
...
AddEventHandler("raid_clothes:set_outfit",function(slot, name, data, cid)
    if not slot then return end
    local src = source
    ...
    exports.ghmattimysql:execute("SELECT count(rank) whitelist FROM jobs_whitelist WHERE cid = @cid LIMIT 1", {
        ...
        exports.ghmattimysql:scalar("SELECT count(model) FROM character_current WHERE cid = @cid LIMIT 1", {
            ...
            if result == 0 then
                ...
                exports.ghmattimysql:execute("SELECT `jail` FROM characters where id = @cid", {['@cid'] = cid}, function(data)
                    if data[1].jail >= 1 then
                        TriggerClientEvent("hotel:createRoom", src, false, false)
                    elseif data[1].jail <= 0 then

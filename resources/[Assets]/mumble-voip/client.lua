...
function StartRadioTask()
    Citizen.CreateThread(function()
        ...
        while mumbleConfig.controls.radio.pressed do
		...
		if initialised then
			...
			if IsControlJustPressed(0, mumbleConfig.controls.proximity.key) then
				...
				if not secondaryPressed then
					...
					if mumbleConfig.voiceModes[voiceMode][2] == "Whisper" then
						TriggerEvent('voip:settizng', 2)
					elseif mumbleConfig.voiceModes[voiceMode][2] == "Normal" then
						TriggerEvent('voip:settizng', 1)
					elseif mumbleConfig.voiceModes[voiceMode][2] == "Shouting" then
						TriggerEvent('voip:settizng', 3)
					end

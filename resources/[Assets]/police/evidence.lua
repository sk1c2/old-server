...
AddEventHandler("denoms", function(denomsp)
  ...
  hasdenoms = denomsp
  if hasdenoms then
	TriggerEvent('chatMessagess', 'STATUS: ', 1, "Your currency has Multiple Denominations")
  else
	TriggerEvent('chatMessagess', 'STATUS: ', 1, "Your currency no longer has Multiple Denominations")
  end
end)
...
function myBleeds()
    ...
    if not initialbleed and math.random(100) < 20 then
        if lightbleed or heavybleed or lightestbleed then
            TriggerEvent("Notification","You notice  oobloodzing from your body the more you move.",2)
            -- SetEntityHealth(PlayerPedId(), GetEntityHealth(PlayerPedId()) - bleedmulti)
        end
    end

    if initialbleed then
        TriggerEvent("Notification","You notice blood oozing from your body the more you move.",2)
        for i = 1, 7 do
             if lightbleed or heavybleed or lightestbleed then
                -- SetEntityHealth(PlayerPedId(), GetEntityHealth(PlayerPedId()) - bleedmulti)
                Citizen.Wait(350)
...
AddEventHandler("Evidence:CurrentDamageListTarget",function(CurrentDamageListTarget,bt,targetid)
	for i = 1, #CurrentDamageListTarget do
		TriggerEvent('chatMessagess', 'STATUS: ', 1, CurrentDamageListTarget[i])
	end
...
AddEventHandler("Evidence:CurrentDamageList",function()
	...
	for i = 1, #CurrentDamageList do
		TriggerEvent('chatMessagess', 'STATUS: ', 1, CurrentDamageList[i])
	end
	
	for i = 1, #bones do
		if bones[i]["timer"] > 0 then
			TriggerEvent('chatMessagess', 'STATUS: ', 1, bones[i]["part"] .. " Injury")
		end
...
AddEventHandler("Evidence:CurrentPainList",function()
	...
	for i = 1, #CurrentLocationList do
		TriggerEvent('chatMessagess', 'STATUS: ', 1, CurrentLocationList[i])
	end
end)
...
function findBed(fadein)
	...
	if not inbed then
		...
		amountofcash = math.random(50, 400)
		if math.random(100) > 97 then
			...
			-- TriggerEvent('customNotification', 'It was a success with complications with your injury, you have been billed for your injuries in the amount of: $' .. amountofcash .. '.')
			-- TriggerEvent('DoLongHudText', 'It was a success with complications with your injury, you have been billed for your injuries in the amount of: $15.')
			-- TriggerEvent('chatMessagess', 'Service ', 5, "It was a success with complications with your " .. myinjury .. ", you have been billed for your injuries in the amount of: $" .. (injurycount*30) .. " and will need to return at a later point! Keep in touch.")
		else
			TriggerEvent("chatMessage","HOSPITAL",5,'It was a success with complications with your injury, you have been billed for your injuries in the amount of: $' .. amountofcash .. '.')
		end
...
AddEventHandler("myboneIssuesTarget",function()
	...
	for i = 1, #bones do
        ...
        if armsdisabled and not legsdisabled then
            TriggerEvent("Notification","I am finding it difficult to use my arms properly.",2)
        elseif legsdisabled and not armsdisabled then
            TriggerEvent("Notification","I am finding it difficult to use my legs properly.",2)
        elseif legsdisabled and armsdisabled then
            TriggerEvent("Notification","I am finding it difficult to use my arms and legs.",2)
        end
        ...
        if damagereport ~= "" then
            TriggerEvent("Notification",damagereport, 155)
        end

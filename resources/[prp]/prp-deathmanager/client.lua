...
AddEventHandler('deathdrop', function(beingDragged)
    ...
    if beingDragged and imDead == 1 then
      ...
      if not beingDragged and imDead == 1 then
        ...
        SetEntityCoords( PlayerPedId(), GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 0.0, 0.0) )
    end 
end)
...
function deathTimer()
    EHeld = 500
    TriggerEvent("urp:alert:downplayer")
    thecount = 300
    TriggerEvent("doTimer")
    TriggerEvent('HEHEHEHEHEHEHEHE')
...
AddEventHandler('trycpr', function()
    ...
    if curDist < 5 or curDist2 < 5 then
        ...
        TriggerServerEvent('admin:revivePlayerClient', GetPlayerServerId(targetplayer))
        TriggerEvent('Crypto:RemoveUnerium', 3)
...
RegisterNetEvent('ReviveHEHEHEHEHHEE')
AddEventHandler('ReviveHEHEHEHEHHEE', function()
    ragdol = 1
    imDead = 0
    thecount = 240
    TriggerEvent("Heal")

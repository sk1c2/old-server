...
AddEventHandler('veh.examine', function(plate)
    ...
    exports.ghmattimysql:execute('SELECT `degredation`, `engine_damage`, `body_damage` FROM __vehicles WHERE plate = @plate', {
        ['@plate'] = plate
      }, function (result) 
...
local matAmount = amount
        local amount = amount*10
            ...
            exports.ghmattimysql:execute('SELECT * FROM __vehicles WHERE plate = @plate', {
                ['@plate'] = plate
            }, function (result) 
            ...
            --print(updateDB)
            exports.ghmattimysql:execute("UPDATE __vehicles SET degredation = @degredation WHERE plate = @plate", {
...
AddEventHandler('veh.updateVehicleHealth', function(tempReturn)
    ...
    exports.ghmattimysql:execute("UPDATE __vehicles SET engine_damage = @engine_damage, body_damage = @body_damage, fuel = @fuel WHERE plate = @plate", {
...
AddEventHandler('veh.updateVehicleDegredationServer', function(plate,br,ax,rad,cl,tra,elec,fi,ft)
    exports.ghmattimysql:execute('SELECT plate FROM __vehicles WHERE plate = @plate', {
        ['@plate'] = plate
      }, function (result)
